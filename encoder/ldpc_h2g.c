...

{
  ...
  mexErrMsgTxt("GFq_m: I'm afraid I don't know how to multiply in GFq\n");
...

{
  ...
  if ( a == 0) mexErrMsgTxt("GFq_inv: no inverse for 0!\n");; 
  ...
  mexErrMsgTxt("GFq_inv: not defined inverse for  GFq\n");
...
/* invert HH matrix here */
	...
	ir = (int *)mxMalloc(M*sizeof(mwIndex));

        jc = (int *)mxMalloc(N*sizeof(mwIndex));
